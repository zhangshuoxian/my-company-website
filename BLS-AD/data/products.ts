...
import { IMAGES } from '../constants'; // Circular dependency warning: we will fix this by moving IMAGES to a separate shared file or importing carefully. 
// For now, let's keep IMAGES in constants.ts and assume we can import it. But constants.ts will import THIS file. 
// To avoid circular dependency, we should define IMAGES here or in a separate file. 
// Let's redefine IMAGES here for now as a local const or import from a new 'data/images.ts' if we were stricter.
// However, since constants.ts imports this, we cannot import constants.ts here.
// SOLUTION: Move IMAGES definition to this file or duplicate it. Duplicating minimal URLs for safety or moving IMAGES to a base file is better.
// I will move IMAGES to a new data/images.ts to be clean.

// Actually, I will just redefine the images object here to avoid complexity of another file for now, 
// as the user wants to split "reference forms".
// Wait, the best way is to have IMAGES in 'data/siteData.ts' or similar, but let's put it here for Products use.
