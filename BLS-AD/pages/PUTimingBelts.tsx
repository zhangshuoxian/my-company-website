...
const PUTimingBelts = () => {
  ...
  const currentId = 'pu-timing-belts';
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex flex-col md:flex-row justify-between items-center">
             <div className="mb-4 md:mb-0">
               ...
               <h1 className="text-2xl font-bold text-brand-blue uppercase">{currentItem?.title || 'PU TIMING BELTS'}</h1>
             </div>
        ...
        <div className="pt-8 md:pt-10 max-w-7xl mx-auto">
          ...
          <Reveal direction="top">
            <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-gray-100 mb-12 border-l-[16px] border-l-brand-blue">
               <h2 className="text-xl font-black text-brand-blue mb-4 uppercase tracking-widest">Technological Introduction</h2>
          ...
          <div className="space-y-12">
            {pageData.tables.map((table, idx) => (
              <Reveal key={idx} direction="bottom" delay={idx * 100}>
                <div className="flex flex-col lg:flex-row gap-10 bg-white p-8 rounded-[3.5rem] shadow-sm border border-gray-100 hover:shadow-xl transition-all">
                   {/* 表格配图 */}
                   <div className="lg:w-1/4 flex items-center justify-center bg-gray-50 rounded-[2.5rem] p-6 border-2 border-gray-50">
                      <img src={table.image} alt={table.title} className="max-w-full h-auto object-contain mix-blend-multiply hover:scale-110 transition-transform duration-500" />
                   </div>
          ...
          {pageData.toleranceText && (
            <Reveal direction="bottom">
              <div className="mt-16 p-8 bg-brand-lightBlue/30 rounded-[3rem] border-2 border-brand-blue/10 flex items-center gap-8">
                <div className="w-16 h-16 bg-brand-blue text-white rounded-full flex items-center justify-center shrink-0 shadow-lg"><RefreshCw size={32}/></div>
                <div>
                   <h4 className="text-xs font-black text-brand-blue uppercase tracking-widest mb-1">Engineering Tolerances</h4>
                   <pre className="font-sans text-gray-800 text-lg font-black whitespace-pre-wrap">{pageData.toleranceText}</pre>
                </div>
              </div>
            </Reveal>
          )}
